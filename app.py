        ...
        if submit_button:
            with st.spinner("Salvando respostas no banco de dados..."):
                try:
                    ...
                    if iciq_6_6: motivos.append("Ao vestir")
                    if iciq_6_7: motivos.append("Sem razão")
                    if iciq_6_8: motivos.append("Tempo todo")
                    motivos_finais = ", ".join(motivos)

                    # Construção da linha (quebrada para evitar cortes)
                    nova_linha = [
                        doenca_previa, idade, sexo, peso, altura, periodo, faculdade, estado,
                        hidratacao, cafeina, 
                        iciq_3, iciq_4, iciq_5, motivos_finais,
                        oab_3a, oab_3b, oab_4a, oab_4b, oab_5a, oab_5b, oab_6a, oab_6b,
                        pss_1, pss_2, pss_3, pss_4, pss_5, pss_6, pss_7, pss_8, pss_9, pss_10,
                        str(psqi_1), psqi_2, str(psqi_3), psqi_4, 
                        psqi_5a, psqi_5b, psqi_5c, psqi_5d, psqi_5e, 
                        psqi_5f, psqi_5g, psqi_5h, psqi_5i, 
                        psqi_6, psqi_7, psqi_8, psqi_9, psqi_10
                    ]
                    
                    # Transformando vazios em textos limpos
                    nova_linha_texto = [str(item) if item is not None else "" for item in nova_linha]
                    
                    # Envio final
                    aba.append_row(nova_linha_texto)
                    ...
                    if "200" in str(erro):
                        ...
                        st.error(f"Erro ao conectar com a planilha. Detalhes: {erro}")
